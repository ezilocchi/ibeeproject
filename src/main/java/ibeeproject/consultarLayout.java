 ...
 *
 * @version consultarLayout.java
 * @version Created on 20-sep-2009, 23:55:31
 * @author Administrador
 */
public class consultarLayout extends AbstractFragmentBean {
    ...
    public String action_eliminar() {
        ...
        gestLay.deleteUno(this.layout);
        gestPos.deleteUno(this.layout);
        e.cargarLayouts();
        return null;
    }
    ...
    public String mostrarPosicion() {
        ...
        try {
            ArrayList botones = new ArrayList();
            int count = 0;
            for (int i = 1; i < this.getLayout().getLargo() + 1; i++) {
                for (int j = 1; j < this.getLayout().getAncho() + 1; j++) {
                    count++;
                    HtmlCommandButton boton = new HtmlCommandButton();
                    boton.setId("a" + count);
                    boton.setValue(i + ":" + j);
